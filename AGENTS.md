...
- Keep multiple independent OKF catalog projects as direct children of this repository.
- Version catalog content, source manifests, source locks, provenance, requirements, and documentation in Git.
...
- After refresh changes normalized documents, use the OKF Manager plugin's concept workflow to update only affected concepts, then run its full catalog pipeline.
- Run `mark-unavailable` only after explicit user direction. Archived-unavailable documents require targeted `refresh --document <id> --include-archived` for manual recovery.
- Report exact project, source, operation, network use, change counts, validation results, and any unverified target behavior.
