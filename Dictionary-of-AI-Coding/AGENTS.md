...
- `_Requirements/` owns project intent.
- `_Documentation/` describes the project source and catalog workflow.
...
- Keep `source.json`, `source.lock.json`, `changes/`, and `plans/` directly under `okf/sources/`.
- Do not manually modify hydrated evidence under `okf/raw/cache/`.
...
- Run portable and hydrated source validation with `_tools/okf_source_manager.py` from the repository root.
...
- `_Requirements/AGENTS.md` governs project requirements.
- `_Documentation/AGENTS.md` governs project documentation.
